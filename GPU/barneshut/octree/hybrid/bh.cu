...
void check_depth(bh_oct_tree_node * node, int _depth);
void track(bh_oct_tree_node * node, int _depth);
static int DEPTH_STAT = 0;
const int nStreams = 4;
int *track_array;
bool *h_correlation_matrix = NULL;
bool *d_correlation_matrix = NULL;
  ...
  int i, junk, c;
	...
	for(i = optind; i < argc; i++) {
		...
		case 1:
			...
			fscanf(infile, "%lld", &junk); // chomp the input size so
			break;
		}
	}
...
void compute_center_of_mass(bh_oct_tree_node *root, int _depth) {
	int i = 0;
	int j = 0;
	float mass;
	vec3d cofm;
	...
	for (i = 0; i < 8; i++) {
		...
		if (child != 0) {
			...
			if (child->type == bhNonLeafNode) {
				// summarize children
				compute_center_of_mass(child, _depth + 1);									
			} else {
...
int main(int argc, char **argv) {
	...
	printf("Configuration Confirmation:\n");
	printf("NUM_OF_WARPS_PER_BLOCK = %d, NUM_OF_BLOCKS = %d, SPLICE_DEPTH = %d\n", NUM_OF_WARPS_PER_BLOCK, NUM_OF_BLOCKS, SPLICE_DEPTH);
#ifdef TRACK_TRAVERSALS
	printf("TRACK_TRAVERSALS = %d\n", TRACK_TRAVERSALS);
#endif
	...
	for (t = 0; t < ntimesteps; t++) {
		...
		// Summarize nodes
		sortidx = 0;
		compute_center_of_mass(root, 0);
		TIME_END(compute_cofm);

		TIME_START(extra);
	    check_depth(root, SPLICE_DEPTH);
		TIME_END(extra);
...
//				printf("the %dth point in h_set has an id = %d.\n", i, points_unsorted[i]->id);
			}
		...
		TIME_RESTART(extra);
		long nMatrixSize = nbodies * DEPTH_STAT;
		CUDA_SAFE_CALL(cudaHostAlloc((void**)&h_correlation_matrix, sizeof(bool) * nMatrixSize, cudaHostAllocDefault));  
	    CUDA_SAFE_CALL(cudaMalloc(&(d_correlation_matrix), sizeof(bool) * nMatrixSize));
		params.d_matrix = d_correlation_matrix;
		...
		long stream_workload = nbodies / nStreams;
		long point_offset = 0;
		long matrix_workload = nMatrixSize / nStreams;
		CUDA_SAFE_CALL( cudaEventRecord(startEvent,0) );
		for (i = 0; i < nStreams; i ++) {
			compute_force_pre_gpu<<<blocks, tpb, 0, stream[i]>>>(params, point_offset, point_offset + stream_workload);
...
//		DEPTH_STAT = 0;
//	    track(root, SPLICE_DEPTH);

		int* DATA = new int [nbodies];
		int data_size = nbodies;
		for(i = 0; i < nbodies; i ++) {
			DATA[i] = i;
		}

		for(int node = 0; node < DEPTH_STAT; node ++)
		{
	//		buffer_index = 0;
	//		memset(cpu_buffer, 0, bytes);

			int* A = new int [nbodies];
			int* B = new int [nbodies];
			int a_size = 0;
			int b_size = 0;
			long index = node * nbodies;
			for (i = 0; i < nbodies; i ++) {
		...
		int buffer_index = 0;
		bh_oct_tree_node **re_sorted = new bh_oct_tree_node*[nbodies];
		for (i = 0; i < nbodies; i ++) {
			re_sorted[buffer_index ++] = params.h_points_sorted[DATA[i]];
		}
		params.h_points_sorted = re_sorted;
		copy_kernel_params_to_device(params);
			
		printf("SPLICE_DEPTH = %d, DEPTH_STAT = %d.\n", SPLICE_DEPTH, DEPTH_STAT);
		delete [] DATA;
		DATA = NULL;
...
//		print_oct_tree(root, 1, 0);
		...
		#ifdef TRACK_TRAVERSALS
		// traversals are lockstep so get by warp
		int nwarps = 0;
		for(i = 0; i < nbodies + (nbodies % 32); i+=32, nwarps++) {			
			bh_oct_tree_node **pts = (sort_flag) ? points_sorted : points_unsorted;
			//printf("nodes warp %d: %d\n", i/32, pts[i]->nodes_accessed);
			sum_nodes_accessed += pts[i]->nodes_accessed;
		}
    ...
    free_kernel_params_device(params);
		delete [] re_sorted;
		CUDA_SAFE_CALL(cudaFreeHost(h_correlation_matrix));
...
void check_depth(bh_oct_tree_node * node, int _depth)
{
	...
	{
		check_depth(node->children[child], _depth);
	}
}
...
void track(bh_oct_tree_node * node, int _depth)
{
	...
	if (node->depth >= _depth)
	{
		...
		int count = 0;
		for(cluster_iter = DATA->begin(); cluster_iter != DATA->end(); cluster_iter ++)
		{
			if(h_correlation_matrix[node->pre_id * nbodies + *cluster_iter])
			{
				A->push_back(*cluster_iter);
				count ++;
			}
			else
			{
				B->push_back(*cluster_iter);
			}
		}
//		printf("pre_id %d, count = %d， DATA->size = %d.\n", node->pre_id, count, DATA->size());
		for(cluster_iter = B->begin(); cluster_iter != B->end(); cluster_iter ++)
		{
			A->push_back(*cluster_iter);
		}
	...
	else
	{
		for (int child = 0; child < 8; child ++)
		{
			track(node->children[child], _depth);
		}
	}
}
